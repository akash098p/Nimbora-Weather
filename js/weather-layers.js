...
async function radar() {
  ...
  try {
    ...
    console.error(e);
    setStatus("Rain radar temporarily unavailable");
  }
}
async function grid() {
  const p = window.meghdootGetPlace?.();
  ...
  for (let a = -2; a <= 2; a++)
    for (let b = -2; b <= 2; b++)
      pts.push([
        +(p.latitude + a * latStep).toFixed(3),
        +(p.longitude + b * lonStep).toFixed(3),
      ]);
  const lat = pts.map((x) => x[0]).join(","),
    lon = pts.map((x) => x[1]).join(",");
  const u = new URL(WX);
  u.search = new URLSearchParams({
    ...
    timezone: "auto",
    current:
      "precipitation,rain,weather_code,wind_speed_10m,wind_direction_10m,wind_gusts_10m",
  });
  try {
    ...
    console.error(e);
    setStatus("Weather layer unavailable");
  }
}
function drawGrid(arr) {
  ...
  arr.forEach((x, i) => {
    ...
    const rain = Number(x.rain ?? x.precipitation ?? 0),
    ...
    if (MeghdootLayers.mode === "rain") {
      ...
      value = Math.min(100, Math.max(50, gust));
      label = `Severe storm risk · gust ${Math.round(gust)} km/h`;
    }
...
async function applyLayer(mode) {
  ...
  if (["wind", "thunder", "cyclone"].includes(mode)) return grid();
}
function setStatus(s) {
  const x = document.querySelector("#interactiveMapStatus");
