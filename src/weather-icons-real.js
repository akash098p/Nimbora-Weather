const RealWeatherIcons=(()=>{
  ...
  const palette={sun:'#FFD86A',moon:'#E8F1FF',cloud:'#F4F7FA',cloudShade:'#C9D5E1',rain:'#76C8FF',snow:'#FFFFFF',bolt:'#FFE66D',fog:'#DCE7EE'};
  function kind(code,isDay){code=Number(code);if(code===0)return isDay?'clear-day':'clear-night';if(code===1)return isDay?'mostly-clear-day':'mostly-clear-night';if(code===2)return isDay?'partly-cloudy-day':'partly-cloudy-night';if(code===3)return 'overcast';if(code===45||code===48)return 'fog';if(code>=51&&code<=57)return 'drizzle';if(code>=61&&code<=67)return 'rain';if(code>=71&&code<=77)return 'snow';if(code>=80&&code<=82)return 'showers';if(code>=85&&code<=86)return 'snow-showers';if(code>=95)return 'thunderstorm';return 'unknown'}
  function svg(type){
    ...
    let body='';
    switch(type){case'clear-day':body=sun;break;case'clear-night':body=moon;break;case'mostly-clear-day':body=sun+cloud;break;case'mostly-clear-night':body=moon+cloud;break;case'partly-cloudy-day':body=sun+cloud;break;case'partly-cloudy-night':body=moon+cloud;break;case'overcast':body=cloud+`<g class="ri-cloud-back"><path d="M34 64h50c11 0 20-8 20-18 0-9-7-17-17-18-4-10-13-16-24-16-10 0-19 6-23 15-14 0-24 9-24 20 0 9 8 17 18 17z" fill="${palette.cloudShade}" opacity=".85"/></g>`;break;case'fog':body=cloud+fog;break;case'drizzle':case'rain':case'showers':body=cloud+rain;break;case'snow':case'snow-showers':body=cloud+snow;break;case'thunderstorm':body=cloud+rain+bolt;break;default:body=sun;}
    return `<svg class="real-weather-svg" viewBox="0 0 110 120" aria-hidden="true">${defs}${body}</svg>`;
  }
  function render(code,isDay){const e=root();if(!e)return;const t=kind(code,isDay);e.className='weather-icon-large real-weather-icon';e.dataset.weather=t;e.innerHTML=svg(t);e.setAttribute('aria-label',t.replaceAll('-',' '));}
  return{render};
})();
